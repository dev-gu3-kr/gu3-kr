...
export function PriestListView({
  items,
  isLoading,
  isError,
  failedImageIds,
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5 text-sm text-neutral-700">
                ...
                <p className="flex items-center gap-1.5">
                  ...
                  <span>
                    <span className="text-neutral-500">축일:</span>{" "}
                    {item.feastMonth ?? "-"}월 {item.feastDay ?? "-"}일
                  </span>
                </p>
                <p className="flex items-center gap-1.5 text-sm text-neutral-600">
                  <Clock3 className="h-4 w-4 text-primary" />
                  <span>
                    <span className="text-neutral-500">재임기간:</span>{" "}
                    {item.termStart ? item.termStart.slice(0, 10) : "-"} ~{" "}
                    {item.termEnd ? item.termEnd.slice(0, 10) : "현재"}
                  </span>
                </p>
              </div>
            </div>
