...
## 관리자 권한 정책
- `User.role = SUPER_ADMIN`: 사용자 관리 메뉴를 포함한 전체 관리 기능 허용
- 일반 관리자: `User.menuPermissions`에 저장된 메뉴만 허용
- 메뉴 권한 하나는 해당 메뉴의 조회·등록·수정·삭제 권한을 모두 포함
- `ADMIN`, `EDITOR`, `VIEWER` 역할 값은 기존 데이터 호환을 위해 유지하지만 세부 메뉴 권한 판단에는 사용하지 않음
